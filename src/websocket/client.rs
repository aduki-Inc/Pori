...
impl WebSocketClient {
    ...
    pub async fn run(&self) -> Result<()> {
        info!("Starting WebSocket client for URL: {}", self.app_state.settings.websocket.url);
        ...
        loop {
            ...
            let _ = self.app_state.dashboard_tx.send(
                DashboardEvent::ConnectionStatus(ConnectionStatus::Connecting)
            );
            ...
            match self.connect_and_run().await {
                ...
                Err(e) => {
                    ...
                    // Update connection status
                    let _ = self.app_state.dashboard_tx.send(
                        DashboardEvent::ConnectionStatus(ConnectionStatus::Disconnected)
                    );
                    ...
                    if delay > Duration::from_secs(0) {
                        ...
                        let _ = self.app_state.dashboard_tx.send(
                            DashboardEvent::ConnectionStatus(ConnectionStatus::Reconnecting)
                        );
    ...
    async fn connect_and_run(&self) -> Result<()> {
        info!("Attempting WebSocket connection to {}", self.app_state.settings.websocket.url);
        ...
        ws_sink.send(Message::Text(auth_json)).await
    ...
    async fn handle_incoming_message(&self, message: Message) -> Result<()> {
        match message {
            Message::Text(text) => {
                ...
                let tunnel_message = TunnelMessage::from_json(&text)
    ...
    pub async fn send_message(&self, message: TunnelMessage) -> Result<()> {
        ...
        if let Some(ref tx) = *tx_guard {
            ...
            debug!("Queueing message for later delivery: {}", message.message_type());
