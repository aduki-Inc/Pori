...
impl ProxyForwarder {
    ...
    pub async fn run(&self, mut message_rx: mpsc::UnboundedReceiver<ProxyMessage>) -> Result<()> {
        ...
        while let Some(message) = message_rx.recv().await {
            match message {
                ProxyMessage::HttpRequest { id, method, url, headers, body } => {
                ...
                _ => {
                    debug!("Ignoring non-HTTP request message: {:?}", message.message_type());
                }
            }
        }
    ...
    async fn handle_http_request(
        ...
        let _ = self.app_state.dashboard_tx.send(
            DashboardEvent::RequestForwarded(format!("{} {}", method, path))
        );
        ...
        match result {
            ...
            Err(e) => {
                ...
                // Notify dashboard
                let _ = self.app_state.dashboard_tx.send(
                    DashboardEvent::Error(format!("Request failed: {}", e))
                );
            }
        }
...
mod tests {
    ...
    fn create_test_app_state() -> Arc<AppState> {
        ...
        args.local = "https://localhost:3000".parse().unwrap();
    ...
    fn test_url_path_extraction() {
        ...
        assert_eq!(forwarder.extract_path_from_url("/api/test").unwrap(), "/api/test");
        assert_eq!(forwarder.extract_path_from_url("api/test").unwrap(), "/api/test");
        
        let full_url_result = forwarder.extract_path_from_url("https://example.com/api/test?param=value");
